...
export const makeNotAcceptedMailText = (
  bodyMail: any,
  operation: any,
  operations: any,
  uvrs: any
  ...
  const uvrsText = () => {
    ...
    ${uvrs
      .map((uvr: any) => {
        return `${uvr.message_id} más inforamción en ${getUrlRfv(
          uvr.message_id
        )}`;
      })
      .join("\n")}`;
...
export const makeNotAcceptedMailHtml = (
  bodyMail: any,
  operation: any,
  operations: any,
  uvrs: any
) => {
  const intersectedOperations = () => {
    return `
    ${paragraph(
      "La misión no ha sido aceptada porque vuela en zonas donde ya existen las siguientes operaciones aprobadas:"
    )}
    ${operations
      .map((op: any, idx: any) => {
        return `
      ${paragraph(
        `<b>${idx}:</b>  <a href="${getPublicOperationLink(op)}">${
          op.name
        }</a><smal>(${getPublicOperationLink(op)})</small>`
      )}
    `;
      })
      .join("")}`;
  ...
  const uvrsText = () => {
    return `
    ${paragraph(
      "La misión no ha sido aceptada porque vuela en zonas donde exiten las siguientes zonas reservadas:"
    )}
    ${uvrs
      .map((uvr: any, idx: any) => {
        return `
      ${paragraph(
        `<a href="${getUrlUvr(uvr.message_id)}">Zona ${
          idx + 1
        }</a> <small>(${getUrlUvr(uvr.message_id)})</small>`
      )}
      `;
      })
      .join("")}
  ...
  const mjmlBody = `
  ${title(bodyMail)}
  ${operations.length > 0 ? intersectedOperations() : ""}
  ${uvrs.length > 0 ? uvrsText() : ""}
  ${bodyOperationMail(operation)}
  `;
  return generateBaseMail(mjmlBody);
