...
export class RestrictedFlightVolumeDao {
  private tabName = "restricted_flight_volume";
  private rfvClass = RestrictedFlightVolume;
  ...
  async getRfvIntersections(
    volume: OperationVolume,
    entManager?: EntityManager
  ) {
    let qBuilder = this.repository.createQueryBuilder(this.tabName);
    if (entManager !== undefined) {
      qBuilder = entManager.createQueryBuilder(this.rfvClass, this.tabName);
    }
    const count = await qBuilder
      .where(
        '(numrange("min_altitude", "max_altitude") && numrange(:min_altitude, :max_altitude)) ' +
          ' AND (ST_Intersects("geography" ,ST_GeomFromGeoJSON(:geom)))'
      )
